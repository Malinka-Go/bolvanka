...
async function mintNFT() {
  ...
  try {
    ...
    const metadataJson = JSON.stringify(metadata); // Без data:application/json
    const metadataCell = new TonWeb.boc.Cell();
    metadataCell.bits.writeString(metadataJson);
    console.log("Metadata cell bits:", metadataCell.bits.toString());
    ...
    const base64Payload = (await payload.toBoc()).toString("base64");
